...
async function readBoundedNoFollow(path, maximumBytes) {
  if (typeof constants.O_NOFOLLOW !== 'number')
    throw new Error('This platform cannot safely inspect packaged Bun provenance.');
  ...
  if (
    ...
    throw new Error('Packaged Bun provenance resource is unsafe.');
...
async function hashBoundedNoFollow(path, maximumBytes) {
  if (typeof constants.O_NOFOLLOW !== 'number')
    throw new Error('This platform cannot safely inspect a packaged Bun archive.');
  const pathBefore = await lstat(path);
  if (
    !pathBefore.isFile() ||
    pathBefore.isSymbolicLink() ||
    ...
    throw new Error('Packaged Bun archive resource is unsafe.');
  const handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
  try {
    const before = await handle.stat();
    if (!before.isFile() || before.size !== pathBefore.size)
      throw new Error('Packaged Bun archive resource changed while being read.');
...
async function packagedBunComponents(resourcesDirectory) {
  ...
  const provenance = JSON.parse(
    (await readBoundedNoFollow(provenancePath, 16 * 1024)).toString('utf8')
  );
  if (
    !provenance ||
    ...
    throw new Error('Packaged Bun provenance resource is invalid.');
  const components = [];
  const selectedRuntimeArchitectures = arch === 'universal' ? ['arm64', 'x64'] : [arch];
  for (const runtimeArch of selectedRuntimeArchitectures) {
    if (runtimeArch !== 'arm64' && runtimeArch !== 'x64')
      throw new Error('Packaged Bun provenance includes an unsupported architecture.');
    const expected = packagedBun[runtimeArch];
    const source = provenance.archives[runtimeArch];
    if (
      !source ||
    ...
    if (
      ...
      source.archiveSha256 !== expected.archiveSha256 ||
      source.binarySha256 !== expected.binarySha256 ||
      source.binaryPath !== expected.binaryPath ||
      source.releaseUrl !==
        `https://github.com/oven-sh/bun/releases/download/bun-v1.3.14/${expected.fileName}`
    )
    ...
    components.push({
      ...
      name: `bun-darwin-${runtimeArch}-archive`,
      version: '1.3.14',
      scope: 'required',
      hashes: [{ alg: 'SHA-256', content: expected.archiveSha256 }],
      properties: [
        {
          name: 'selene:runtime-evidence',
          value: `resources/bun/${runtimeArch}/${expected.fileName}`
        },
...
if (platform === 'macos') components.push(...(await packagedBunComponents(dirname(archivePath))));
