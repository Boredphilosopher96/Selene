...
export function validateGeneratedProjectToolchainManifest(value: unknown): GeneratedProjectToolchainManifest {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('generated project toolchain manifest is invalid');
  const manifest = value as Record<string, unknown>;
  if (manifest.format !== 'selene-generated-project-toolchain/v1') throw new Error('generated project toolchain format is invalid');
  const packages = manifest.packages;
  if (typeof packages !== 'object' || packages === null || Array.isArray(packages)) throw new Error('generated project package manifest is invalid');
  const names = ['react', 'reactDom', 'vite', 'viteReact', 'typescript', 'storybook', 'storybookReactVite', 'storybookAddonA11y'] as const;
  const packageRecord = packages as Record<string, unknown>;
  const version = (name: string, candidate: unknown): string => {
    if (typeof candidate !== 'string' || !exactSemver.test(candidate)) throw new Error(`generated project ${name} must be an exact semantic version`);
    return candidate;
  };
  const normalized = Object.fromEntries(names.map((name) => [name, version(name, packageRecord[name])])) as GeneratedProjectToolchainManifest['packages'];
  const bunVersion = version('Bun', manifest.bunVersion);
  if (normalized.react !== normalized.reactDom) throw new Error('generated project react and react-dom versions must match');
  if (normalized.storybook !== normalized.storybookReactVite || normalized.storybook !== normalized.storybookAddonA11y)
