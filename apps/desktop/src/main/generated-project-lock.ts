...
export const BUN_LOCK_ONLY_ARGS = Object.freeze([
  'install',
  '--lockfile-only',
  '--ignore-scripts',
  '--no-progress',
...
export const BUN_1_3_14_EXECUTABLE_ATTESTATIONS: Readonly<
  Record<BunExecutableAttestation['arch'], BunExecutableAttestation>
> = Object.freeze({
  arm64: Object.freeze({
    bunVersion: '1.3.14',
    arch: 'arm64',
...
export interface VerifiedBunRuntimePort {
  resolve(options: { readonly signal: AbortSignal }): Promise<
    Readonly<{
      executable: string;
...
export interface GeneratedProjectRegistryPolicyPort {
  ...
  bunConfiguration(context: { readonly bundleDigest: string; readonly filePlanDigest: string }): {
    readonly registryUrl?: string;
  };
}

export class NoGeneratedProjectRegistryPolicy implements GeneratedProjectRegistryPolicyPort {
  public bunConfiguration(_context: {
...
export class GeneratedProjectCommandError extends Error {
  public constructor(
    ...
    public readonly cleanupScope:
      'generated-project-lease' | 'runtime-stage' = 'generated-project-lease'
  ) {
    super(message);
  }
}
...
const stableMessages: Readonly<Record<PublishAdapterError['code'], string>> = Object.freeze({
  ...
  PROCESS_ORPHANED:
    'Generated project process termination could not be confirmed; host recovery is required.',
...
export class HostAttestedBunCommandPort implements GeneratedProjectCommandPort {
    ...
    try {
      ...
      if (!stat.isDirectory() || stat.isSymbolicLink())
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project host directory is unsafe.'
        );
      ...
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project host directory is unavailable.'
      );
    }
  }

  private async cacheDirectory(trustDigest: string): Promise<string> {
    if (!/^[a-f0-9]{64}$/.test(trustDigest))
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project registry trust domain is invalid.'
      );
    ...
    if (
      relative(root, cacheRoot) !== cacheNamespace ||
      relative(cacheRoot, cache) !== `registry-${trustDigest}`
    )
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project cache path is unsafe.'
      );
    for (const candidate of [cacheRoot, cache]) {
      ...
      if (!stat.isDirectory() || stat.isSymbolicLink() || !underRoot(actual))
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project cache directory is unsafe.'
        );
    }
  ...
  private async executable(
    ...
    try {
      if (!isAbsolute(bunExecutable) || attestation.arch !== process.arch)
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'No verified Bun executable is packaged for this architecture.'
        );
      ...
      if (!configured.isFile() || configured.isSymbolicLink() || (configured.mode & 0o111) === 0)
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project Bun executable is unsafe.'
        );
      const actual = await realpath(bunExecutable);
      const configuredAfterResolution = await lstat(bunExecutable);
      if (
        configuredAfterResolution.isSymbolicLink() ||
        configuredAfterResolution.dev !== configured.dev ||
        configuredAfterResolution.ino !== configured.ino
      )
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project Bun executable changed while being verified.'
        );
      ...
      if (typeof constants.O_NOFOLLOW !== 'number')
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'This platform cannot safely verify the Bun executable.'
        );
      ...
      try {
        ...
        if (
          ...
          hash.digest('hex') !== attestation.executableSha256
        )
          throw new GeneratedProjectCommandError(
            'TOOL_UNAVAILABLE',
            'Generated project Bun executable does not match packaged provenance.'
          );
      } finally {
        await handle.close();
      }
      ...
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project Bun executable is unavailable.'
      );
    }
  }

  private async leaseConfigurationPath(
    materialization: GeneratedProjectMaterialization
  ): Promise<string> {
    try {
      ...
      if (relative(materialization.root, configDirectory) !== 'selene')
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project Bun configuration path is unsafe.'
        );
      ...
      if (
        ...
        actualConfigDirectory !== configDirectory
      )
        throw new GeneratedProjectCommandError(
          'TOOL_UNAVAILABLE',
          'Generated project Bun configuration path is unsafe.'
      ...
      if (
        relative(materialization.root, path) !== 'selene/.bunfig.lock-only.toml' ||
      ...
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project Bun configuration path is unavailable.'
      );
    }
  }

  private async bunConfiguration(
    materialization: GeneratedProjectMaterialization,
    context: { readonly bundleDigest: string; readonly filePlanDigest: string }
  ): Promise<ResolvedBunRegistry> {
    const configured = this.registryPolicy.bunConfiguration(context);
    if (
      typeof configured !== 'object' ||
      configured === null ||
      Array.isArray(configured) ||
      Object.keys(configured).some((key) => key !== 'registryUrl')
    )
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project registry policy is invalid.'
      );
    const requestedRegistry = configured.registryUrl ?? defaultRegistry;
    if (
      typeof requestedRegistry !== 'string' ||
      requestedRegistry.length === 0 ||
      requestedRegistry.length > 2_048
    )
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project registry policy is invalid.'
    ...
    const trustDigest = createHash('sha256')
      .update(`selene-generated-project-registry/v1\u0000${normalizedRegistry}`)
      .digest('hex');
    ...
    try {
      ...
      await handle.writeFile(registry, 'utf8');
      await handle.sync();
    } catch {
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project Bun configuration could not be prepared.'
      );
    } finally {
  ...
  public async runBunLockOnly(
    ...
    if (context.signal.aborted)
      throw new GeneratedProjectCommandError(
        'CANCELLED',
        'Generated project lock generation was cancelled.'
      );
    let runtime: Readonly<{ executable: string; attestation: BunExecutableAttestation }>;
    try {
      runtime = await this.runtime.resolve({ signal: context.signal });
    } catch (error) {
      if (error instanceof VerifiedBunRuntimeError)
        throw new GeneratedProjectCommandError(
          error.code,
          stableMessages[error.code],
          error.processGroupId,
          error.cleanupScope
        );
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Verified packaged Bun runtime is unavailable.'
      );
    }
    ...
    if (!cwdStat.isDirectory() || cwdStat.isSymbolicLink() || cwd !== materialization.root)
      throw new GeneratedProjectCommandError(
        'TOOL_UNAVAILABLE',
        'Generated project working directory is not an attested lease root.'
      );
    const registry = await this.bunConfiguration(materialization, {
      bundleDigest: context.bundleDigest,
      filePlanDigest: context.filePlanDigest
    });
    ...
    for (const name of ['LANG', 'LC_ALL', 'LC_CTYPE', 'NO_COLOR'] as const)
    ...
    await new Promise<void>((resolvePromise, rejectPromise) => {
      ...
      if (context.signal.aborted) {
        rejectPromise(
          new GeneratedProjectCommandError(
            'CANCELLED',
            'Generated project lock generation was cancelled.'
          )
        );
      ...
      const child = spawn(
        executable,
        [...BUN_LOCK_ONLY_ARGS, '--cache-dir', cache, '--config', registry.config],
        {
          cwd,
          env: environment,
          shell: false,
      ...
      let pending: GeneratedProjectCommandError | undefined;
      ...
      const macProcessGroupExists = (): boolean => {
        if (process.platform !== 'darwin' || child.pid === undefined || child.pid <= 0)
          return false;
        try {
      ...
      const signalTree = (signal: NodeJS.Signals) => {
        try {
          ...
          /* The process may have already closed; close/error settles the operation. */
        }
      };
      ...
      const scheduleGroupProbe = () => {
        if (settled || process.platform !== 'darwin') {
          finishAfterGroup();
          return;
        }
        if (!macProcessGroupExists()) {
          finishAfterGroup();
          return;
        }
        ...
        watchdog = setTimeout(() => {
          if (process.platform === 'darwin' && macProcessGroupExists()) {
            finish(
              new GeneratedProjectCommandError(
                'PROCESS_ORPHANED',
                'Generated project Bun process group termination could not be confirmed.',
                child.pid
              )
            );
            return;
      ...
      timeout = setTimeout(
        () =>
          terminate(
            new GeneratedProjectCommandError(
              'TIMEOUT',
              'Generated project Bun lock generation timed out.'
            )
          ),
        this.timeoutMs
      );
      ...
      const consume = (stream: NodeJS.ReadableStream, kind: 'stdout' | 'stderr') =>
        stream.on('data', (chunk: Buffer) => {
          if (kind === 'stdout') stdout += chunk.byteLength;
          else stderr += chunk.byteLength;
          if (stdout > maximumCommandOutputBytes || stderr > maximumCommandOutputBytes)
            terminate(
              new GeneratedProjectCommandError(
                'PROCESS_FAILED',
                'Generated project Bun output exceeded its bound.'
              )
            );
        });
      ...
      child.once('error', () => {
        ...
        if (!terminating)
          finish(
            new GeneratedProjectCommandError(
              'TOOL_UNAVAILABLE',
              'Generated project Bun process could not start.'
            )
          );
      });
      child.once('close', (code, signal) => {
        ...
        if (pending === undefined && (code !== 0 || signal !== null))
          pending = new GeneratedProjectCommandError(
            'PROCESS_FAILED',
            'Generated project Bun lock generation failed.'
          );
        if (process.platform === 'darwin' && macProcessGroupExists()) {
          ...
          if (!terminating)
            terminate(
              new GeneratedProjectCommandError(
                'PROCESS_FAILED',
                'Generated project Bun process group retained unexpected descendants.'
              )
            );
          scheduleGroupProbe();
          return;
...
export interface GeneratedProjectLockPort {
  ...
  ): Promise<GeneratedProjectLockReceipt>;
}
...
export class BunLockOnlyGeneratedProjectLockPort implements GeneratedProjectLockPort {
  public constructor(
    ...
    private readonly command: GeneratedProjectCommandPort
  ) {}
  ...
  public async resolve(
    ...
    if (
      validated.filePlanDigest !== materialization.filePlanDigest ||
      validated.bundle.digest !== materialization.bundleDigest
    )
      throw new PublishAdapterError(
        'CONFLICT',
        'Generated project lease does not match the immutable publish plan.'
      );
    try {
      await this.materializer.assertLease(materialization);
    } catch {
      throw new PublishAdapterError('INTEGRITY', stableMessages.INTEGRITY);
    }
    await this.command.runBunLockOnly(materialization, {
    ...
    try {
      ...
      throw new PublishAdapterError(
        'INTEGRITY',
        'Lock-only validation must not create node_modules.'
      );
    } catch (error) {
    ...
    if (
      relative(materialization.root, lock) !== 'bun.lock' ||
      typeof constants.O_NOFOLLOW !== 'number'
    )
      throw new PublishAdapterError('INTEGRITY', 'Generated Bun lockfile path is unsafe.');
    let handle: Awaited<ReturnType<typeof open>> | undefined;
    try {
    ...
    if (lockHandle === undefined)
      throw new PublishAdapterError('INTEGRITY', 'Generated Bun lockfile is unavailable.');
    try {
      const stat = await lockHandle.stat();
      if (!stat.isFile() || stat.size <= 0 || stat.size > 1_024 * 1_024)
        throw new PublishAdapterError('INTEGRITY', 'Generated Bun lockfile is invalid.');
      const content = await lockHandle.readFile();
      if (content.byteLength !== stat.size)
        throw new PublishAdapterError(
          'INTEGRITY',
          'Generated Bun lockfile changed while being read.'
        );
      const lockDigest = createHash('sha256').update(content).digest('hex');
      const artifactDigest = createHash('sha256')
        .update(`${validated.bundle.digest}\u0000${validated.filePlanDigest}\u0000${lockDigest}`)
        .digest('hex');
...
export class LocalGeneratedProjectValidationAdapter implements GeneratedCodePublishPort {
  ...
  public async publish(
    ...
    if (request.mode !== 'local-preview')
      throw new PublishAdapterError(
        'CONFLICT',
        'The local validation adapter cannot publish remotely.'
      );
    if (
    ...
    try {
      ...
      materialization = await this.materializer.materialize(request.plan, {
        signal: options.signal
      });
      ...
      return {
        ...
        filePlanDigest: receipt.filePlanDigest,
        artifactDigest: receipt.artifactDigest,
        validation: 'materialized-lock',
