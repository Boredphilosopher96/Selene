...
export class PrototypeGraphConflictError extends Error {
  ...
  public constructor() {
    super('The flow graph changed in another workspace. Reload and retry.');
  }
}
...
export class PrototypeGraphPersistenceError extends Error {
  public constructor(
    public readonly code:
      'GRAPH_PERSISTENCE_READ' | 'GRAPH_PERSISTENCE_CORRUPT' | 'GRAPH_PERSISTENCE_UNSAFE',
    message: string,
...
async function graphLock<T>(key: string, work: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    release();
    if (graphLocks.get(key) === chain) graphLocks.delete(key);
  }
}
...
export class JsonPrototypeGraphPersistencePort implements PrototypeGraphPersistencePort {
  ...
  private path(projectId: string): string {
    ...
    if (relative(root, candidate).startsWith('..'))
      throw new Error('project path escaped persistence root');
    return candidate;
  }
  private recoveryPath(projectId: string, recoveryId: string): string {
    ...
    if (relative(root, candidate).startsWith('..'))
      throw new Error('recovery path escaped persistence root');
    return candidate;
  }
  private recoveryMarkerPath(projectId: string): string {
    ...
    if (relative(root, candidate).startsWith('..'))
      throw new Error('recovery marker escaped persistence root');
    return candidate;
  }
  private static isRecoveryId(value: unknown): value is string {
    return (
      typeof value === 'string' &&
      /^graph-recovery-[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i.test(value)
    );
  }
  ...
  private async openNoFollow(path: string) {
    if (typeof constants.O_NOFOLLOW !== 'number')
      throw new PrototypeGraphPersistenceError(
        ...
        'This platform cannot safely open saved graph files.'
      );
    try {
      ...
      if (code === 'ELOOP' || code === 'ENOTSUP' || code === 'EOPNOTSUPP')
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_UNSAFE',
          'Saved graph path is unsafe or no-follow is unsupported.'
        );
      throw error;
    }
  }
  private async boundedRaw(
    path: string
  ): Promise<{ readonly bytes: Buffer; readonly originalBytes: number }> {
    ...
    try {
      ...
      if (!metadata.isFile())
        throw new PrototypeGraphPersistenceError(
          ...
          'Saved graph is not a regular file.'
        );
      if (metadata.size > JsonPrototypeGraphPersistencePort.maxGraphBytes)
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_CORRUPT',
          'Saved graph exceeds the 256 KiB parsing limit.'
        );
      ...
      if (bytesRead !== bytes.length)
        throw new PrototypeGraphPersistenceError(
          ...
          'Saved graph changed while being read.'
        );
  ...
  private async recoveryMarker(
    projectId: string
  ): Promise<{ readonly recoveryId: string } | undefined> {
    try {
      ...
      if (
        ...
        !JsonPrototypeGraphPersistencePort.isRecoveryId(
          (value as Record<string, unknown>).recoveryId
        )
      )
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_CORRUPT',
          'Saved graph recovery marker is invalid.'
        );
      ...
      throw new PrototypeGraphPersistenceError(
        'GRAPH_PERSISTENCE_READ',
        'Saved graph recovery status could not be read.'
      );
    }
  }
  private async recoveryEvidence(path: string): Promise<{
    ...
    try {
      ...
      if (!metadata.isFile())
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_CORRUPT',
          'Recovered graph is not a regular file.'
        );
      const prefix = Buffer.alloc(
        Math.min(metadata.size, JsonPrototypeGraphPersistencePort.maxGraphBytes)
      );
      const first = await file.read(prefix, 0, prefix.length, 0);
      if (first.bytesRead !== prefix.length)
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_READ',
          'Recovered graph changed while being recorded.'
        );
      ...
      for (let offset = 0; offset < metadata.size; offset += chunk.length) {
        ...
        const { bytesRead } = await file.read(
          chunk,
          0,
          Math.min(chunk.length, metadata.size - offset),
          offset
        );
        if (bytesRead === 0)
          throw new PrototypeGraphPersistenceError(
            'GRAPH_PERSISTENCE_READ',
            'Recovered graph changed while hashing.'
          );
  ...
  public async read(projectId: string): Promise<PersistedPrototypeGraph | undefined> {
    try {
      ...
      if (!Number.isSafeInteger(parsed.revision) || parsed.revision < 1)
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_CORRUPT',
          'Saved graph revision is invalid.'
        );
      try {
        ...
        throw new PrototypeGraphPersistenceError(
          'GRAPH_PERSISTENCE_CORRUPT',
          'Saved graph contents are invalid.'
        );
      }
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined;
...
export class PublishAdapterError extends Error {
  public constructor(
    public readonly code: PublishAdapterErrorCode,
    message: string
  ) {
    super(message);
  }
...
function canonicalJson(value: unknown, seen = new Set<object>()): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      throw new Error('immutable publish bundle cannot contain a non-finite number');
...
export function serializeImmutablePublishBundle(
  bundle: Omit<ImmutablePublishBundle, 'bundleDigest' | 'immutableId'>
): string {
...
function validateImmutablePublishBundleInput(input: ImmutablePublishBundleInput): void {
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(input.projectId))
    throw new Error('publish bundle project ID is invalid');
  validateReactSourceWorkspace(input.source);
  if (input.source.projectId !== input.projectId)
    throw new Error('publish bundle workspace project does not match project ID');
  ...
  if (collaboration.project.id !== input.projectId)
    throw new Error('publish bundle collaboration project does not match project ID');
  if (
    input.designInputProvenance.format !== 'selene-desktop-current-workspace-design-inputs/v1' ||
    input.designInputProvenance.projectId !== input.projectId
  )
  ...
  const digest = (value: unknown, name: string) => {
    if (typeof value !== 'string' || !/^[a-f0-9]{64}$/.test(value))
      throw new Error(`publish bundle ${name} digest is invalid`);
  };
  ...
  for (const input of stagedSystems) {
    ...
    text(stagedSystem.version, 'design-system version', 128);
    digest(stagedSystem.artifactDigest, 'design-system');
    if (!Array.isArray(stagedSystem.exports) || stagedSystem.exports.length > 1_024)
      throw new Error('publish bundle design-system exports are invalid');
    for (const entry of stagedSystem.exports) text(entry, 'design-system export', 512);
    text(stagedSystem.provenance.provider, 'design-system provider', 256);
  ...
  if (stagedLanguage !== undefined) {
    if (stagedLanguage.status !== 'staged')
      throw new Error('publish bundle design-language receipt is invalid');
    digest(stagedLanguage.artifactDigest, 'design-language');
    text(stagedLanguage.provenance.provider, 'design-language provider', 256);
    text(stagedLanguage.provenance.location, 'design-language location', 2_048);
    if (
      !Number.isSafeInteger(stagedLanguage.sectionCount) ||
      stagedLanguage.sectionCount < 0 ||
      stagedLanguage.sectionCount > 65_536
    )
      throw new Error('publish bundle design-language section count is invalid');
  }
  if (
    !Array.isArray(input.componentCatalog.entries) ||
    input.componentCatalog.entries.length > 4_096
  )
    throw new Error('publish bundle component catalog is invalid');
  for (const entry of input.componentCatalog.entries)
    if (
      !entry ||
      typeof entry.component !== 'string' ||
      entry.component.length === 0 ||
      entry.component.length > 256 ||
      typeof entry.href !== 'string' ||
      entry.href.length > 2_048
    )
  ...
  if (!/^[a-f0-9]{64}$/.test(input.packageProvenance.lockfile.checksum))
    throw new Error('publish bundle lockfile checksum is invalid');
  const packageList = (
    ...
    for (const entry of entries) {
      if (!entry || typeof entry !== 'object')
        throw new Error(`publish bundle ${name} entry is invalid`);
...
export function createImmutablePublishBundle(
  input: ImmutablePublishBundleInput
): ImmutablePublishBundle {
  ...
  const bundleDigest = createHash('sha256')
    .update(serializeImmutablePublishBundle(content))
    .digest('hex');
...
export interface HostedStakeholderReviewPort {
  synchronize(
    publication: HostedStakeholderReviewPublication,
    signal: AbortSignal
  ): Promise<HostedStakeholderReviewAdapterResult>;
}
function reviewDigest(value: string): string {
...
function validArtifactSha(value: unknown): value is string {
  return typeof value === 'string' && /^[a-f0-9]{40}$/.test(value);
}
function validPullRequestUrl(value: unknown, repository: string): value is string {
  try {
    return canonicalGitHubPullRequestUrl(value, repository) === value;
  } catch {
    return false;
  }
}
...
export function createHostedStakeholderReviewPublication(
  ...
  if (
    ...
    throw new PublishAdapterError(
      'INTEGRITY',
      'Hosted review publication does not match the immutable artifact.'
    );
  let repository: string;
  try {
    repository = canonicalGitHubRepository(receipt.repository);
  } catch {
    throw new PublishAdapterError('INTEGRITY', 'Hosted review repository is invalid.');
  }
  if (
    !validArtifactRef(receipt.ref) ||
    !validArtifactSha(receipt.commitSha) ||
    !validArtifactSha(receipt.treeSha) ||
    !validPullRequestUrl(receipt.pullRequestUrl, repository)
  )
...
export class UnconfiguredHostedStakeholderReviewPort implements HostedStakeholderReviewPort {
  public async synchronize(
    ...
    if (signal.aborted)
      throw new PublishAdapterError('CANCELLED', 'Hosted review synchronization was cancelled.');
...
export class PublishAdapterRegistry {
  ...
  public constructor(adapters: readonly GeneratedCodePublishPort[]) {
    for (const adapter of adapters) {
      if (this.adapters.has(adapter.mode))
        throw new Error(`duplicate publish adapter mode: ${adapter.mode}`);
  ...
  public select(mode: GeneratedCodePublishPort['mode']): GeneratedCodePublishPort {
    ...
    if (!adapter)
      throw new PublishAdapterError(
        'CONFLICT',
        `No host publish adapter is configured for ${mode}.`
      );
    return adapter;
  }
}
...
export function publishConsentDigest(binding: PublishConsentBinding): string {
...
export class FixturePublishConsentPort implements TrustedPublishConsentPort {
  ...
  public async consume(consentId: string, binding: PublishConsentBinding): Promise<void> {
    if (this.grants.get(consentId) !== publishConsentDigest(binding))
      throw new PublishAdapterError('AUTH_REQUIRED', 'Explicit host publish consent is required.');
...
export class DeterministicLocalPublishAdapter implements GeneratedCodePublishPort {
  ...
  public async publish(
    ...
    if (request.mode !== this.mode)
      throw new PublishAdapterError(
        'CONFLICT',
        'The selected publish adapter does not support this mode.'
      );
    if (options.signal.aborted) throw new PublishAdapterError('CANCELLED', 'Publish cancelled.');
    options.progress(
      'Fixture-validated the immutable local publish bundle without project materialization.'
    );
    await Promise.resolve();
    if (options.signal.aborted) throw new PublishAdapterError('CANCELLED', 'Publish cancelled.');
    const artifactDigest = createHash('sha256')
      .update(`${request.bundle.bundleDigest}\u0000${request.plan.filePlanDigest}\u0000fixture`)
      .digest('hex');
    return {
      mode: 'local-preview',
