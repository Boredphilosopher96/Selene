...
export interface GeneratedProjectFilePlan {
  ...
  readonly template: { readonly id: string; readonly version: string };
  readonly bundle: { readonly immutableId: string; readonly digest: string; readonly sourceEntrypoint: string };
  readonly toolchain: GeneratedProjectToolchainManifest;
...
export interface GeneratedProjectTemplatePort {
  readonly id: string;
  readonly version: string;
  create(bundle: ImmutablePublishBundle, request?: GeneratedProjectTemplateRequest): GeneratedProjectFilePlan;
}
...
function plainJson(value: unknown, seen = new Set<object>()): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error('generated project data cannot contain non-finite numbers');
  ...
  const result = `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${plainJson(record[key], seen)}`).join(',')}}`;
  seen.delete(value);
  return result;
}
...
function catalogNodes(bundle: ImmutablePublishBundle) {
  ...
  for (const node of bundle.source.nodes) nodes.set(`${node.path}\u0000${node.exportName}`, { path: node.path, exportName: node.exportName });
  return [...nodes.values()].sort((left, right) => comparePath(`${left.path}\u0000${left.exportName}`, `${right.path}\u0000${right.exportName}`));
}

function storyFile(node: { readonly path: string; readonly exportName: string }): GeneratedProjectFile {
  ...
  const componentImport = node.exportName === 'default'
...
function packageJson(bundle: ImmutablePublishBundle, toolchain: GeneratedProjectToolchainManifest): string {
  ...
  return json({
    name: `selene-${bundle.projectId.toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '') || 'generated-project'}`,
    private: true,
    version: '0.0.0',
    type: 'module',
    packageManager: `bun@${toolchain.bunVersion}`,
    engines: { bun: toolchain.bunVersion },
    scripts: { dev: 'vite', build: 'vite build', storybook: 'storybook dev -p 6006', 'build-storybook': 'storybook build' },
    dependencies,
    devDependencies: {
...
function requiredFiles(bundle: ImmutablePublishBundle, toolchain: GeneratedProjectToolchainManifest): readonly GeneratedProjectFile[] {
  const entryNode = bundle.source.nodes.find((node) => node.path === bundle.source.entrypoint && node.exportName === 'default')
    ?? bundle.source.nodes.find((node) => node.path === bundle.source.entrypoint);
  if (entryNode === undefined) throw new Error('generated workspace entrypoint has no component export metadata');
  const entryImport = normalizeImportPath('src/main.tsx', entryNode.path);
  const entryComponent = entryNode.exportName === 'default'
  ...
  return [
    ...
    { path: 'tsconfig.json', content: json({ compilerOptions: { target: 'ES2022', useDefineForClassFields: true, lib: ['ES2022', 'DOM', 'DOM.Iterable'], allowJs: false, skipLibCheck: true, esModuleInterop: true, allowSyntheticDefaultImports: true, strict: true, module: 'ESNext', moduleResolution: 'Bundler', resolveJsonModule: true, isolatedModules: true, noEmit: true, jsx: 'react-jsx' }, include: ['src', '.storybook'] }) },
...
export function validateGeneratedProjectFilePlan(value: unknown): GeneratedProjectFilePlan {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('generated project file plan is invalid');
  const plan = value as GeneratedProjectFilePlan;
  if (plan.format !== 'selene-generated-project-file-plan/v1' || !digest.test(plan.bundle?.digest ?? '') || typeof plan.bundle?.immutableId !== 'string' || !stableIdentifier.test(plan.template?.id ?? '') || !exactSemver.test(plan.template?.version ?? ''))
  ...
  if (!Array.isArray(plan.contributions) || plan.contributions.length > 64) throw new Error('generated project contribution plan is invalid');
  const contributionIds = new Set<string>();
  for (const contribution of plan.contributions) {
    if (!contribution || !stableIdentifier.test(contribution.id) || !exactSemver.test(contribution.version) || (contribution.kind !== 'user-template' && contribution.kind !== 'design-system') || typeof contribution.provenance?.provider !== 'string' || contribution.provenance.provider.length === 0 || contribution.provenance.provider.length > 256 || !digest.test(contribution.provenance.digest) || contributionIds.has(`${contribution.id}\u0000${contribution.version}`))
      throw new Error('generated project contribution plan is invalid');
    contributionIds.add(`${contribution.id}\u0000${contribution.version}`);
  }
  if (!Array.isArray(plan.files) || plan.files.length === 0 || plan.files.length > maxFiles) throw new Error('generated project file count is invalid');
  ...
  if (!paths.includes('package.json') || !paths.includes('src/main.tsx') || !paths.includes(plan.bundle.sourceEntrypoint)) throw new Error('generated project scaffolding is incomplete');
