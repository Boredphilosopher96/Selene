...
export class GitHubGeneratedProjectPublishAdapter implements GeneratedCodePublishPort {
  ...
  public async publish(request: GeneratedCodePublishRequest, options: { readonly signal: AbortSignal; readonly progress: (message: string) => void }): Promise<GeneratedCodePublishReceipt> {
    ...
    try {
      ...
      return Object.freeze({ mode: 'github-remote', status: 'remote-published', repository: repositoryName, bundleDigest: request.bundle.bundleDigest, filePlanDigest: request.plan.filePlanDigest, lockDigest: locked.lockDigest, artifactDigest: locked.artifactDigest, treeSha: verifiedTree.sha, commitSha: verifiedCommit.sha, ref: verifiedRef.ref, pullRequestUrl: pullRequest.html_url, immutableId: request.bundle.immutableId, hostedReview: Object.freeze({ status: 'pending', reason: 'HOSTED_REVIEW_NOT_PROVISIONED' }) });
