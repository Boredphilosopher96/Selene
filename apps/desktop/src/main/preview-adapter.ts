...
const policy=Object.freeze({origin:decode(root.dataset.previewOrigin),nonce:decode(root.dataset.previewNonce),revisionId:decode(root.dataset.previewRevisionId)});
...
let painted=false;let renderFailure;const reportPaint=()=>{if(painted)report('rendered')};const reportFailure=()=>{if(renderFailure)report('runtime-error',{message:renderFailure})};const fail=message=>{renderFailure=(String(message).trim()||'Preview module could not render').slice(0,4000);reportFailure()};
const initialize=event=>{if(!event.isTrusted||event.source!==window.parent||port)return;apply(stopImmediate,event,[]);const value=fields(event.data,['type','nonce','revisionId']);if(!value||value.type!=='selene-preview-init'||value.nonce!==policy.nonce||value.revisionId!==policy.revisionId||event.ports.length!==1)return;removeWindowListener('message',initialize,true);port=event.ports[0];apply(addPortListener,port,['message',event=>{if(closed)return;const message=channel(event.data);if(!message)return;if(message.type==='canvas-navigation'){canvasNavigationEnabled=message.enabled;return}if(message.type==='target-cancel'){targetCancelEnabled=message.enabled;return}window.dispatchEvent(new CustomEvent('selene-runtime-state',{detail:message.state}))}]);apply(startPort,port,[]);report('ready');reportFailure();reportPaint()};
...
const elementTelemetry=node=>{const style=computedStyle(node);const rect=apply(getBounds,node,[]);const attribute=name=>(apply(getAttribute,node,[name])||'').slice(0,256);return {width:Math.max(0,rect.width),height:Math.max(0,rect.height),display:style.display.slice(0,256),position:style.position.slice(0,256),boxSizing:style.boxSizing.slice(0,256),margin:style.margin.slice(0,256),padding:style.padding.slice(0,256),gap:style.gap.slice(0,256),flexDirection:style.flexDirection.slice(0,256),alignItems:style.alignItems.slice(0,256),justifyContent:style.justifyContent.slice(0,256),gridTemplateColumns:style.gridTemplateColumns.slice(0,256),gridTemplateRows:style.gridTemplateRows.slice(0,256),overflow:style.overflow.slice(0,256),fontFamily:style.fontFamily.slice(0,256),fontSize:style.fontSize.slice(0,256),fontWeight:style.fontWeight.slice(0,256),lineHeight:style.lineHeight.slice(0,256),letterSpacing:style.letterSpacing.slice(0,256),textAlign:style.textAlign.slice(0,256),textDecoration:style.textDecoration.slice(0,256),color:style.color.slice(0,256),backgroundColor:style.backgroundColor.slice(0,256),border:style.border.slice(0,256),borderRadius:style.borderRadius.slice(0,256),boxShadow:style.boxShadow.slice(0,512),opacity:style.opacity.slice(0,256),semanticTag:node.tagName.toLowerCase().slice(0,256),explicitAriaRole:attribute('role'),ariaLabel:attribute('aria-label'),accessibleDescription:(attribute('aria-description')||attribute('title')).slice(0,256),ariaDisabled:attribute('aria-disabled'),ariaExpanded:attribute('aria-expanded'),ariaPressed:attribute('aria-pressed'),ariaChecked:attribute('aria-checked'),ariaSelected:attribute('aria-selected'),ariaHidden:attribute('aria-hidden'),tabIndex:node instanceof HTMLElement?Math.max(-1,Math.min(32767,node.tabIndex)):-1}};
document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(!target)return;const node=target.closest('[data-selene-node-id]');if(canvasNavigationEnabled&&node){apply(preventDefault,event,[]);apply(stopImmediate,event,[]);const nodeId=node.getAttribute('data-selene-node-id')||'';if(identifier.test(nodeId))report('select-node',{nodeId,telemetry:elementTelemetry(node)});return}const action=target.closest('[data-selene-flow-node][data-selene-action-port]');if(action){const nodeId=action.getAttribute('data-selene-flow-node')||'';const portId=action.getAttribute('data-selene-action-port')||'';if(identifier.test(nodeId)&&identifier.test(portId))report('trigger-action',{nodeId,portId});return}if(node){const nodeId=node.getAttribute('data-selene-node-id')||'';if(identifier.test(nodeId))report('select-node',{nodeId,telemetry:elementTelemetry(node)})}},{capture:true});
