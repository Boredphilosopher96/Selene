...
const identifier=/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/;let port;let closed=false;let canvasNavigationEnabled=false;let canvasPointerSelection=false;let targetCancelEnabled=false;
...
addWindowListener('click',event=>{if(!canvasPointerSelection||!canvasNavigationEnabled||!event.isTrusted){if(!canvasNavigationEnabled)canvasPointerSelection=false;return}canvasPointerSelection=false;apply(preventDefault,event,[]);apply(stopImmediate,event,[])},{capture:true});addWindowListener('pointercancel',event=>{if(event.isTrusted&&event.isPrimary)canvasPointerSelection=false},{capture:true});document.addEventListener('pointerdown',event=>{canvasPointerSelection=false;if(!canvasNavigationEnabled||!event.isTrusted||!event.isPrimary||event.button!==0)return;const target=event.target instanceof Element?event.target:null;if(!target)return;const inspected=target.closest('[data-selene-node-id]')||target;canvasPointerSelection=true;apply(preventDefault,event,[]);apply(stopImmediate,event,[]);const nodeId=apply(getAttribute,inspected,['data-selene-node-id'])||'';if(identifier.test(nodeId)){report('select-node',{nodeId,telemetry:elementTelemetry(inspected)});return}inspectElementSequence+=1;report('inspect-element',{elementId:'unmapped-'+inspectElementSequence,telemetry:unmappedElementTelemetry(inspected)})},{capture:true});
