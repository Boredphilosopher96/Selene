...
document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(!target)return;const action=target.closest('[data-selene-flow-node][data-selene-action-port]');const markedNode=target.closest('[data-selene-node-id]');if(!canvasNavigationEnabled&&action){const nodeId=action.getAttribute('data-selene-flow-node')||'';const portId=action.getAttribute('data-selene-action-port')||'';if(identifier.test(nodeId)&&identifier.test(portId))report('trigger-action',{nodeId,portId});return}if(canvasNavigationEnabled){const nodeId=apply(getAttribute,target,['data-selene-node-id'])||'';apply(preventDefault,event,[]);apply(stopImmediate,event,[]);if(identifier.test(nodeId)){report('select-node',{nodeId,telemetry:elementTelemetry(target)});return}inspectElementSequence+=1;report('inspect-element',{elementId:'unmapped-'+inspectElementSequence,telemetry:unmappedElementTelemetry(target)});return}if(markedNode){const nodeId=markedNode.getAttribute('data-selene-node-id')||'';if(identifier.test(nodeId))report('select-node',{nodeId,telemetry:elementTelemetry(markedNode)})}},{capture:true});
