...
const report=(type,extra={})=>{if(closed)return;const message={type,origin:policy.origin,nonce:policy.nonce,revisionId:policy.revisionId,...extra};if(port)apply(postMessage,port,[message]);if(type==='clear-selection')window.parent.postMessage(message,'*')};
...
addWindowListener('click',event=>{if(!canvasPointerSelection||!canvasNavigationEnabled||!event.isTrusted){if(!canvasNavigationEnabled)canvasPointerSelection=false;return}canvasPointerSelection=false;apply(preventDefault,event,[]);apply(stopImmediate,event,[])},{capture:true});addWindowListener('pointercancel',event=>{if(event.isTrusted&&event.isPrimary){canvasPointerSelection=false;pendingCanvasSelection=undefined;suppressUnsupportedClick=false}},{capture:true});addWindowListener('pointerup',event=>{const pending=pendingCanvasSelection;pendingCanvasSelection=undefined;if(!pending||!canvasNavigationEnabled||!event.isTrusted||!event.isPrimary||event.button!==0)return;report(pending.type,pending.extra)},{capture:true});document.addEventListener('pointerdown',event=>{canvasPointerSelection=false;pendingCanvasSelection=undefined;suppressUnsupportedClick=false;if(!event.isTrusted||!event.isPrimary||event.button!==0)return;const target=event.target instanceof Element?event.target:null;if(!target)return;const markedNode=target.closest('[data-selene-node-id]');if(!canvasNavigationEnabled){if(markedNode||target.closest('[data-selene-flow-node][data-selene-action-port]'))return;suppressUnsupportedClick=true;report('clear-selection');inspectElementSequence+=1;report('inspect-element',{elementId:'unmapped-'+inspectElementSequence,telemetry:unmappedElementTelemetry(target)});return}const inspected=markedNode||target;canvasPointerSelection=true;apply(preventDefault,event,[]);apply(stopImmediate,event,[]);const nodeId=apply(getAttribute,inspected,['data-selene-node-id'])||'';if(identifier.test(nodeId)){pendingCanvasSelection={type:'select-node',extra:{nodeId,telemetry:elementTelemetry(inspected)}};return}report('clear-selection');inspectElementSequence+=1;report('inspect-element',{elementId:'unmapped-'+inspectElementSequence,telemetry:unmappedElementTelemetry(inspected)})},{capture:true});
