...
function selectionProofRejection(headers: Readonly<Record<string, string>>): Response {
...
const nativeUrl=URL;
const trustedPreviewUrl=new nativeUrl(window.location.href);
const selectionKeyEndpoint=new nativeUrl('./selection-key',trustedPreviewUrl).href;
const selectionProofEndpoint=new nativeUrl('./selection-proof',trustedPreviewUrl).href;
...
const postSelf=window.postMessage.bind(window);
...
const clearFromNativeBridge=event=>{if(!event.isTrusted||event.source!==window||!canvasNavigationEnabled)return;const value=fields(event.data,['type','nonce','revisionId']);if(!value||value.type!=='selene-preview-native-clear'||value.nonce!==policy.nonce||value.revisionId!==policy.revisionId)return;report('clear-selection')};
addWindowListener('message',initialize,true);addWindowListener('message',clearFromNativeBridge,true);addWindowListener('pagehide',()=>{closed=true;canvasPointerSelection=undefined;removeWindowListener('message',initialize,true);removeWindowListener('message',clearFromNativeBridge,true);if(commitObserver)apply(disconnectMutation,commitObserver,[]);if(port)apply(closePort,port,[]);port=undefined},{once:true});addWindowListener('error',event=>fail(event.message),true);
  ...
  public async handle(request: Request | string): Promise<Response> {
    ...
    if (resource === 'selection-proof') {
      ...
      if (
        ...
        return selectionProofRejection(headers);
      try {
        const body = await request.text();
