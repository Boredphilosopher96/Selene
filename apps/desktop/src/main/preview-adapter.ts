...
const channel=value=>{const next=fields(value,['type','nonce','origin','revisionId','state','enabled','nodeId','x','y']);if(!next||next.nonce!==policy.nonce||next.origin!==policy.origin||next.revisionId!==policy.revisionId)return null;if(next.type==='canvas-navigation'||next.type==='target-cancel')return typeof next.enabled==='boolean'&&next.state===undefined&&next.nodeId===undefined&&next.x===undefined&&next.y===undefined?Object.freeze({type:next.type,enabled:next.enabled}):null;if(next.type==='selection-point')return next.enabled===undefined&&next.state===undefined&&next.nodeId===undefined&&typeof next.x==='number'&&finite(next.x)&&next.x>=0&&next.x<=1&&typeof next.y==='number'&&finite(next.y)&&next.y>=0&&next.y<=1?Object.freeze({type:next.type,x:next.x,y:next.y}):null;if(next.type==='inspect-node')return next.enabled===undefined&&next.state===undefined&&next.x===undefined&&next.y===undefined&&typeof next.nodeId==='string'&&identifier.test(next.nodeId)?Object.freeze({type:next.type,nodeId:next.nodeId}):null;if(next.type!=='runtime-state'||next.enabled!==undefined||next.nodeId!==undefined||next.x!==undefined||next.y!==undefined)return null;const state=fields(next.state,['activeNodeId','activeStateId','activeOverlayId','activePathTransitionIds']);if(!state||typeof state.activeNodeId!=='string'||!identifier.test(state.activeNodeId)||(state.activeStateId!==undefined&&(typeof state.activeStateId!=='string'||!identifier.test(state.activeStateId)))||(state.activeOverlayId!==undefined&&(typeof state.activeOverlayId!=='string'||!identifier.test(state.activeOverlayId))))return null;const activePathTransitionIds=identifiers(state.activePathTransitionIds,256);if(!activePathTransitionIds)return null;return Object.freeze({type:'runtime-state',state:Object.freeze({activeNodeId:state.activeNodeId,...(state.activeStateId?{activeStateId:state.activeStateId}:{}),...(state.activeOverlayId?{activeOverlayId:state.activeOverlayId}:{}),activePathTransitionIds})})};
