...
let runtimeMounted=false;let latestRuntime;let acceptInitialRuntime;const initialRuntime=new TrustedPromise(resolve=>{acceptInitialRuntime=resolve});const applyRuntime=state=>{latestRuntime=state;if(runtimeMounted)dispatchWindow(new TrustedCustomEvent('selene-runtime-state',{detail:state}))};
const initialize=event=>{if(!event.isTrusted||event.source!==window.parent||port)return;apply(stopImmediate,event,[]);const value=fields(event.data,['type','nonce','revisionId','state']);const state=value?channel({type:'runtime-state',nonce:value.nonce,origin:policy.origin,revisionId:value.revisionId,state:value.state}):null;if(!value||value.type!=='selene-preview-init'||value.nonce!==policy.nonce||value.revisionId!==policy.revisionId||!state||event.ports.length!==1)return;removeWindowListener('message',initialize,true);port=event.ports[0];applyRuntime(state);apply(addPortListener,port,['message',message=>{if(closed)return;const next=channel(message.data);if(next)applyRuntime(next)}]);apply(startPort,port,[]);acceptInitialRuntime();report('ready');reportFailure();reportPaint()};
