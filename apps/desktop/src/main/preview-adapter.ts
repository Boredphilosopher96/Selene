...
const policy=Object.freeze({origin:decode(root.dataset.previewOrigin),nonce:decode(root.dataset.previewNonce),revisionId:decode(root.dataset.previewRevisionId)});let previewCommitted=false;let pendingRuntimeState;const dispatchRuntimeState=state=>window.dispatchEvent(new CustomEvent('selene-runtime-state',{detail:state}));
...
const initialize=event=>{if(!event.isTrusted||event.source!==window.parent||port)return;apply(stopImmediate,event,[]);const value=fields(event.data,['type','nonce','revisionId']);if(!value||value.type!=='selene-preview-init'||value.nonce!==policy.nonce||value.revisionId!==policy.revisionId||event.ports.length!==1)return;removeWindowListener('message',initialize,true);port=event.ports[0];apply(addPortListener,port,['message',event=>{if(closed)return;const message=channel(event.data);if(!message)return;if(message.type==='canvas-navigation'){canvasNavigationEnabled=message.enabled;return}if(message.type==='target-cancel'){targetCancelEnabled=message.enabled;return}if(!previewCommitted){pendingRuntimeState=message.state;return}dispatchRuntimeState(message.state)}]);apply(startPort,port,[]);report('ready');reportFailure();reportPaint()};
...
document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(!target)return;const action=target.closest('[data-selene-flow-node][data-selene-action-port]');const node=target.closest('[data-selene-node-id]');if(!canvasNavigationEnabled&&action){const nodeId=action.getAttribute('data-selene-flow-node')||'';const portId=action.getAttribute('data-selene-action-port')||'';if(identifier.test(nodeId)&&identifier.test(portId))report('trigger-action',{nodeId,portId});return}if(canvasNavigationEnabled&&node){apply(preventDefault,event,[]);apply(stopImmediate,event,[]);const nodeId=node.getAttribute('data-selene-node-id')||'';if(identifier.test(nodeId))report('select-node',{nodeId,telemetry:elementTelemetry(node)});return}if(node){const nodeId=node.getAttribute('data-selene-node-id')||'';if(identifier.test(nodeId))report('select-node',{nodeId,telemetry:elementTelemetry(node)})}},{capture:true});
