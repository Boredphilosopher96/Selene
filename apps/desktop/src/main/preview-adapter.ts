...
let canvasPointerSelection=false;addWindowListener('click',event=>{if(!canvasPointerSelection)return;canvasPointerSelection=false;apply(preventDefault,event,[]);apply(stopImmediate,event,[])},{capture:true});document.addEventListener('pointerdown',event=>{if(!canvasNavigationEnabled||!event.isTrusted)return;const target=event.target instanceof Element?event.target:null;if(!target)return;const inspected=target.closest('[data-selene-node-id]')||target;canvasPointerSelection=true;apply(preventDefault,event,[]);apply(stopImmediate,event,[]);const nodeId=apply(getAttribute,inspected,['data-selene-node-id'])||'';if(identifier.test(nodeId)){report('select-node',{nodeId,telemetry:elementTelemetry(inspected)});return}inspectElementSequence+=1;report('inspect-element',{elementId:'unmapped-'+inspectElementSequence,telemetry:unmappedElementTelemetry(inspected)})},{capture:true});
