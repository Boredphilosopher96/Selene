...
const initialize=event=>{if(!event.isTrusted||event.source!==window.parent||port)return;apply(stopImmediate,event,[]);const value=fields(event.data,['type','nonce','revisionId','canvasNavigationEnabled']);if(!value||value.type!=='selene-preview-init'||value.nonce!==policy.nonce||value.revisionId!==policy.revisionId||typeof value.canvasNavigationEnabled!=='boolean'||event.ports.length!==1)return;removeWindowListener('message',initialize,true);canvasNavigationEnabled=value.canvasNavigationEnabled;port=event.ports[0];apply(addPortListener,port,['message',event=>{if(closed)return;const message=channel(event.data);if(!message)return;if(message.type==='canvas-navigation'){canvasNavigationEnabled=message.enabled;return}if(message.type==='target-cancel'){targetCancelEnabled=message.enabled;return}window.dispatchEvent(new CustomEvent('selene-runtime-state',{detail:message.state}))}]);apply(startPort,port,[]);report('ready');reportFailure();reportPaint()};
