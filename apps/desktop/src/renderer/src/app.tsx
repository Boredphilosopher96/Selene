...
import { assertDesignerApiVersion, defaultWorkspaceCockpitPreferences, type DesignerProgress, type DesignerPublishConsentInput, type DesignerSnapshot, type GeneratedCodePublishReceipt, type WorkspaceCockpitPreferences } from '../../shared/designer-api';
...
export function App() {
  ...
  const startPublish = useCallback(async (request: DesignerPublishConsentInput): Promise<void> => {
    if (publishStartInFlight.current || publishActive) throw new Error('A publish start is already active.');
    ...
    try {
      ...
      const operation = await window.selene.designer.publishGeneratedCode({ ...request, consentId: consent.consentId });
      setPublishId(operation.id);
      setPublishActive(true);
      setPublishStatus(request.mode === 'github-remote' ? 'Remote host operation started; waiting for its immutable receipt.' : 'Local immutable bundle validation started; no files will be retained.');
    } catch (error) {
      setPublishStatus(error instanceof Error && error.message.length > 0 ? error.message : 'Publish was not started.');
      throw error;
    } finally {
  ...
  const cancelPublish = useCallback(async (): Promise<void> => {
    if (publishStartInFlight.current) throw new Error('Trusted host consent is pending; no publish operation can be cancelled yet.');
