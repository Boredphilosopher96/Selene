...
export function GuidedSetupPanel({ snapshot, onSnapshot, onProjectOpened, actions }: GuidedSetupPanelProps) {
    ...
    <section aria-label="Guided local setup">
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) =>
            void actions.selectAgent(event.currentTarget.value).then(onSnapshot).catch((error: unknown) => failure(error, 'Could not select the trusted agent.'))
          }
        >
          {snapshot.agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.label}</option>)}
        </select>
      </label>
      <button type="button" onClick={() => void actions.configureTrustedAgent().then(async (agents) => {
      ...
      <button type="button" onClick={() => void actions.ingestDesignLanguage({ markdown: designMarkdown }).then((receipt) => setStatus(`${receipt.status}: ${receipt.sectionCount} sections from ${receipt.provenance.provider}; digest ${receipt.artifactDigest.slice(0, 12)}.`)).catch((error: unknown) => failure(error, 'Markdown intake failed.'))}>
        Stage Markdown
      </button>
      ...
      <button type="button" onClick={() => void actions.createProject({ id: projectId, name: projectName, template }).then((opened) => open(opened, 'Opened')).catch((error: unknown) => failure(error, 'Project creation failed.'))}>
      ...
      <button type="button" disabled={!projectImport} onClick={() => void actions.importProject({ contents: projectImport }).then((opened) => open(opened, 'Opened imported')).catch((error: unknown) => failure(error, 'Project import failed.'))}>
        Import project
      </button>
