...
describe('canvas workspace interaction model', () => {
  ...
  it('does not expose terminal formatting or local paths in compact canvas errors', () => {
    expect(
      plainCanvasStatus('\u001B[31mENOENT /Users/designer/project\u001B[0m', 'Try again.')
    ).toBe('Try again.');
    expect(plainCanvasStatus('The destination screen is unavailable.')).toBe(
