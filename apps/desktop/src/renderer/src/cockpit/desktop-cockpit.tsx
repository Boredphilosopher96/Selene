...
export function DesktopCockpit({ snapshot, build, frame, onFrameLoad, onSnapshot, onRender, onProjectOpened, actions, guidedActions, progress, preferences, onPreferencesChange, initialSelectedThreadId }: DesktopCockpitProps) {
  ...
  const createReviewThread = (invoking: HTMLElement) => {
    ...
    setGraphSaveStatus('Saving stakeholder review thread…');
    ...
    }).catch((error: unknown) => setGraphSaveStatus(error instanceof Error ? error.message : 'Could not create stakeholder review thread.')).finally(() => setReviewSubmitting(false));
  };
  const requestTargetedChange = () => {
    if (!aiTarget || aiSubmitting) return;
    setAiSubmitting(true);
    setGraphSaveStatus('Applying targeted AI change…');
    void actions.requestAIChange({ agentId: snapshot.selectedAgentId, instruction, target: aiTarget }).then(async (next) => {
      ...
      setGraphSaveStatus(`Applied ${next.source.revision.id}.`);
    }).catch((error: unknown) => setGraphSaveStatus(error instanceof Error ? error.message : 'AI request failed.')).finally(() => setAiSubmitting(false));
  };
  const replyToSelectedThread = async (id: string, body: string): Promise<void> => {
    ...
    try {
      ...
      onSnapshot(next);
      setReplyBody('');
      setGraphSaveStatus('Stakeholder reply saved.');
    } catch (error) { setGraphSaveStatus(error instanceof Error ? error.message : 'Could not reply to review thread.'); }
  ...
  const resolveSelectedThread = async (id: string, resolved: boolean): Promise<void> => {
    ...
    try {
      ...
      onSnapshot(next);
      setGraphSaveStatus(resolved ? 'Stakeholder thread resolved.' : 'Stakeholder thread reopened.');
    } catch (error) { setGraphSaveStatus(error instanceof Error ? error.message : 'Could not update review thread.'); }
    finally { setThreadAction('idle'); }
  };
