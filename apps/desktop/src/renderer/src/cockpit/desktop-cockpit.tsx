  ...
  return (
    <div className="workspace-layout" style={{ '--workspace-left-rail': `${leftWidth}px`, '--workspace-right-rail': `${rightWidth}px` } as CSSProperties} data-left-collapsed={leftCollapsed || undefined} data-right-collapsed={rightCollapsed || undefined}>
      ...
      <aside className="inspector" aria-label="Progressive inspector">
        ...
        {inspectorTab === 'reviews' ? <section id="inspector-reviews" role="tabpanel" aria-labelledby="inspector-tab-reviews"><h2>Stakeholder review</h2><button type="button" onClick={() => setTargeting((value) => !value)}>{targeting ? 'Selecting review location in preview' : 'Pick review location'}</button><p>{target ? `Review location: ${(target.x * 100).toFixed(0)}%, ${(target.y * 100).toFixed(0)}%` : 'Choose a free point or region in the preview before creating a pin or thread.'}</p><label>Stakeholder thread<textarea aria-label="Stakeholder review thread body" value={reviewBody} onChange={(event) => setReviewBody(event.currentTarget.value)} /></label><button type="button" disabled={!target} onClick={() => { if (!target) return; apply(actions.addReviewThread({ body: reviewBody, anchor: target })); }}>Add stakeholder thread</button><button type="button" disabled={!target} onClick={(event) => { if (!target) return; void actions.addArtifactPin({ label: 'Pinned visual region', anchor: target }).then((next) => { onSnapshot(next); const pin = next.artifactPins.at(-1); if (pin) selectArtifactPin(pin.id, event.currentTarget); setGraphSaveStatus('Pinned artifact region.'); }).catch((error: unknown) => setGraphSaveStatus(error instanceof Error ? error.message : 'Could not pin artifact region.')); }}>Pin selected artifact region</button><h2>Spatial review threads</h2>{snapshot.reviewThreads.map((thread) => <button className="review-thread-row" key={thread.id} type="button" aria-pressed={selectedThreadId === thread.id} onClick={(event) => selectThread(thread.id, event.currentTarget)}><strong>{thread.status}</strong><span>{thread.body}</span><small>{thread.replies.length} replies</small></button>)}<h2>Artifact pins</h2>{snapshot.artifactPins.map((pin) => <button key={pin.id} type="button" aria-pressed={selectedArtifactPinId === pin.id} onClick={(event) => selectArtifactPin(pin.id, event.currentTarget)}>{pin.label}: {Math.round(pin.anchor.x * 100)}%, {Math.round(pin.anchor.y * 100)}%</button>)}</section> : null}
