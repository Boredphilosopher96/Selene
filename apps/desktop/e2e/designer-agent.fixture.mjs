      ...
      operations: [
        {
          ...
          path: 'src/App.tsx',
          content:
            "import {useEffect,useLayoutEffect,useState} from 'react'; import './preview.css';\nexport default function App(){const [screen,setScreen]=useState('dashboard');const orders=screen==='orders';useLayoutEffect(()=>{window.history.replaceState({screen:'dashboard'},'', '/');},[]);useEffect(()=>{const onPopState=()=>setScreen(window.location.pathname==='/orders'?'orders':'dashboard');window.addEventListener('popstate',onPopState);return()=>window.removeEventListener('popstate',onPopState)},[]);useEffect(()=>{const captureStopper=event=>{if(event.ctrlKey)event.stopImmediatePropagation()};window.addEventListener('wheel',captureStopper,{capture:true});return()=>window.removeEventListener('wheel',captureStopper,{capture:true})},[]);const openOrders=()=>{window.history.pushState({screen:'orders'},'', '/orders');setScreen('orders')};const stopComponentWheel=event=>{if(event.ctrlKey)event.stopPropagation()};return <main data-selene-node-id=\"designer.root\"><h1 data-selene-node-id=\"designer.title\">{orders?'Orders':'Configured agent dashboard'}</h1><p data-selene-node-id=\"designer.summary\">Configured JSONL process applied the targeted change.</p>{orders?<button data-selene-flow-node=\"orders\" data-selene-action-port=\"back\" data-selene-node-id=\"designer.back-navigation\" onWheel={stopComponentWheel} onClick={()=>window.history.back()}>Back to dashboard</button>:<button data-selene-flow-node=\"dashboard\" data-selene-action-port=\"open-orders\" data-selene-node-id=\"designer.action\" onWheel={stopComponentWheel} onClick={openOrders}>Open orders</button>}</main>}\n"
        }
      ]
    }
  });
