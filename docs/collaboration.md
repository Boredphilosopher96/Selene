...
### Hosted OIDC provider configuration

Hosted mode uses the maintained [`openid-client`](https://github.com/panva/openid-client)
6.8.4 runtime for discovery, Authorization Code + `S256` PKCE, ID-token/JWT
signature validation, issuer/audience/nonce/expiry checks, and JWKS cache
rotation. Selene does not parse JWTs, calculate PKCE challenges, or validate a
signature itself.
...
provider's issuer-qualified stable subject (`<issuer>|<sub>`) in
`users.external_subject`; login only resolves an
active provisioned user and cannot grant a membership by email or silently
create an account.

`GET /auth/login` creates a five-minute, one-time BFF transaction and sets an
opaque `__Host-selene_oidc_tx` cookie. `/auth/callback` consumes it before token
exchange, rotates it into an unrelated `__Host-selene_session` identifier, and
redirects only to a validated same-origin relative path. Both cookies are
...
provider-claimed HTTPS callback delivered to the main-process completion
